#!/usr/bin/env bash
# SKVOZNOJ-PROGON-VOZMOZhNOSTEJ.sh — проверка пути ЦЕЛИКОМ, а не деталей.
#
...
#
# Запуск: bash SKVOZNOJ-PROGON-VOZMOZhNOSTEJ.sh   (из каталога пакета)
...
export HARNESS_CONF="$PKG/harness/config/harness.conf"
...
chk "[ -f '$T/proj/harness/skills/mobile-testing/SKILL.md' ]" "скилы набора появились"
chk "grep -q 'name: android-env' '$T/proj/harness/skills/android-env/SKILL.md'" "скил android-env собран"
chk "grep -q 'Когда' '$T/proj/harness/skills/mobile-release/SKILL.md'" "у скила есть условие срабатывания"
...
out=$(bash "$T/proj/harness/demons/disk-cleanup.sh" --dry-run 2>&1)
