...
[ -r "$HARNESS_CONF" ] || { echo "server-hygiene: нет $HARNESS_CONF — скопируйте харнес/config/harness.conf в /etc/harness/"; exit 1; }
...
Traceback — в $LOG_DIR/server-hygiene.log; чинить: харнес/demons/server_hygiene_check.py"; then
