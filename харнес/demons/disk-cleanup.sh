...
[ -r "$HARNESS_CONF" ] || { echo "disk-cleanup: нет $HARNESS_CONF — скопируйте харнес/config/harness.conf в /etc/harness/"; exit 1; }
...
CAPS_DIR="${PROJECT_DIR:-}/харнес/возможности"
caps_cleaned=0
...
# из харнес/config/удержание.yaml: два источника разъехались бы молча.
RETENTION="$PROJECT_DIR/харнес/config/удержание.yaml"
LOG_KEEP=""
if [ -r "$RETENTION" ]; then
    LOG_KEEP=$(python3 -c "
import yaml, io, sys
...
if [ -n "$LOG_KEEP" ] && [ "$LOG_KEEP" -gt 0 ] 2>/dev/null; then
    ...
    INV="$PROJECT_DIR/харнес/config/инварианты.yaml"
    KEEP_LOGS=$(python3 -c "
