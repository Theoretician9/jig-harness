...
if [ "${1:-}" = "--selftest" ]; then
    ...
    trap 'rm -rf "$T"' EXIT
    ok_flag=1
    mkdir -p "$T/bin" "$T/log" "$T/hb" "$T/proekt/scripts" "$T/proekt/харнес/config" "$T/proekt/docs"
...
MAP
    ...
    cat > "$T/proekt/харнес/config/модели.yaml" <<'MODELI'
модели:
  ревизия: opus
  дежурный: haiku
умолчание: haiku
MODELI
    cat > "$T/proekt/харнес/config/ревизия.yaml" <<'CONF'
...
CONF
    ...
    progon() {  # $1 = действие, $2 = довод демона, далее — пары КЛЮЧ=ЗНАЧЕНИЕ конфига
        ...
        env HARNESS_INSTALL_CONF="$T/install.conf" HARNESS_CONF="$T/harness.proba.conf" \
            ...
            TG_LOG="$T/tg.log" REVIZIA_CONF="${PROBA_CONF:-$T/proekt/харнес/config/ревизия.yaml}" \
...
while IFS= read -r OBLAST; do
    ...
    VOPROS="$(python3 - "$OBLAST" "${REVIZIA_CONF:-$PROJECT_DIR/харнес/config/ревизия.yaml}" <<'PY'
import sys, yaml
