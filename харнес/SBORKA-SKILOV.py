...
затем перегенерация:
    ...
    python3 харнес/SBORKA-SKILOV.py [путь-к-каталогу-глав]
...
SKILLS = {
    ...
    "установка-расширений": (
        "Когда наступило условие из таблицы кандидатов (первый фронт, первый "
        "запрос документа, первая внешняя библиотека, закрытая приёмка, пропуски "
        "скилов в журнале) или в задаче появилось мобильное/клиентское приложение "
        "— установить расширение или крупный набор («возможность») по процедуре",
...
ЗАМЕНЫ = {
    ...
    "templates/spec-template.md": "харнес/шаблоны-задач/spec-template.md",
    "templates/plan-template.md": "харнес/шаблоны-задач/plan-template.md",
    "templates/pretask-template.yaml": "харнес/шаблоны-задач/pretask-template.yaml",
    ...
    "templates/": "харнес/шаблоны-задач/",
}
...
def скилы_возможностей() -> dict:
    """Скилы УСТАНОВЛЕННЫХ возможностей (харнес/возможности/<id>/skills-src/*.md).
    ...
    """
    root = Path(__file__).resolve().parent / "возможности"
    собранные = {}
    if not root.is_dir():
        return собранные
