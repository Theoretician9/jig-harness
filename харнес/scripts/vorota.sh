...
проверка "вход панели"    python3 харнес/панель/test_vhod.py
проверка "служба панели"  python3 харнес/панель/test_server.py
проверка "подтверждение панели" python3 харнес/панель/test_podtverzhdenie.py
проверка "витрина панели"  python3 харнес/панель/test_sostoyanie.py
...
if [ "${1:-}" = "--всё" ]; then
    echo "--- самотесты ---"
    for f in харнес/demons/sentinel.sh харнес/demons/session-warden.sh \
                харнес/demons/backup.sh \
                харнес/demons/heartbeat-watch.sh харнес/demons/task-closer.sh \
                харнес/demons/memory-revision.sh \
                харнес/demons/obnovlenie-watch.sh \
                харнес/demons/tokens-collector.sh scripts/tg-dispatcher.sh \
