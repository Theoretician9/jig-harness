...
EOF
    ...
    echo "САМОТЕСТ ПРОЙДЕН: done без tested_at — красный с текстом И-4, done с tested_at — зелёный, «принято» без команды — красный, без даты — красный, с обоими — зелёный, дубль ключа — красный"
    exit 0
fi
...
CAPS_DIR="${PROJECT_DIR:-$PWD}/харнес/возможности"
caps_gates=0
if [ -d "$CAPS_DIR" ]; then
...
# умрёт при следующем разложить-харнес.sh — молча. Улика 11.08.2026: четыре
