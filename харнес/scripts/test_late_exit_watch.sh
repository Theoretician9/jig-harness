...
#
# Запуск: bash харнес/demons/test_late_exit_watch.sh
set -euo pipefail

# Переехал из харнес/demons/ 10.09.2026 (ревизия): тест в каталоге демонов не
# запускал никто — ни cron, ни systemd, ни ворота. Теперь он рядом с другими
# проверками и входит в «vorota.sh --всё».
DAEMON="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/харнес/demons/session-warden.sh"
T=$(mktemp -d); ok=1
