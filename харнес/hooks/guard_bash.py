...
def pack_bans(cmd: str) -> str | None:
    ...
    Возможность (харнес/возможности/<id>/) может принести свои запреты файлом
    ...
    if not project_dir:
        return None
    root = os.path.join(project_dir, "харнес", "возможности")
...
def check(cmd: str, вложенная: bool = False) -> int:
    global ПРОВЕРЯЕМАЯ
    ...
    for payload in nested_shell_payloads(cmd):
        code = check(payload, вложенная=True)
        if code:
            return code
    ...
    if suicide and not ротация_ведётся_сторожем():
        return deny(
            ...
            "  харнес/demons/session-warden.sh --rotate-now\n"
    ...
    # Только на ВНЕШНЕЙ команде: законная форма — «timeout 600 bash -c
    # '<цикл>'», и её нагрузка, разобранная отдельно, предела в себе не несёт.
    if not вложенная:
        бессрочное = ожидание_без_предела(cmd)
        if бессрочное:
