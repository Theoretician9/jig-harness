...
EOF
    ...
    # Машинный файл: те же числа потребителям кодом. Сверяется с отчётом
    # величинами, а не наличием, — иначе он мог бы разойтись с ним молча.
    MACHINE="$T/log/tokens-daily.json"
    [ -f "$MACHINE" ] || { echo "САМОТЕСТ ПРОВАЛЕН: tokens-daily.json не создан"; exit 1; }
    python3 - "$MACHINE" <<'PROVERKA' || exit 1
import json, sys
числа = json.load(open(sys.argv[1], encoding="utf-8"))
ждали = {"шагов": 6, "с вызовом инструмента": 2, "вызовов на шаг": 2.0,
         "доля одиночных шагов": 0.5, "контекст на шаг": 85,
         "медиана контекста": 7, "90-й процентиль": 300, "95-й процентиль": 300,
         "максимум контекста": 300, "кэш прочитано": 150, "кэш записано": 5,
         "свежий вход": 357, "контекст всего": 512,
         "доля попаданий кэша": 0.293, "ответов всего": 38,
         "ответов на шаг": 6, "прогонов гейтов": 2, "красных гейтов": 1,
         "дата": "2026-06-15"}
разошлись = {к: (в, числа.get(к)) for к, в in ждали.items() if числа.get(к) != в}
if разошлись:
    print(f"САМОТЕСТ ПРОВАЛЕН: машинный отчёт разошёлся (ждали, вышло): {разошлись}")
    sys.exit(1)
PROVERKA
    # Права: панель работает под своим пользователем и читает этот файл.
    [ "$(stat -c '%a' "$MACHINE")" = "644" ] \
        || { echo "САМОТЕСТ ПРОВАЛЕН: tokens-daily.json не читается панелью: $(stat -c '%a' "$MACHINE")"; exit 1; }
...
# Те же числа машине: панель и прочие потребители берут их отсюда, а не
# выковыривают из подписей отчёта.
machine_path = os.path.join(log_dir, "tokens-daily.json")
RECENT_MAX = 500  # окно глобально помнимых пар дедупа (П-3)

def write_atomic(path, text, mode=0o600):
    """mkstemp создаёт файл 0600 — и суточный ОТЧЁТ о расходе оказывался
    нечитаемым для службы панели (она работает под своим пользователем).
    Владелец видел в панели «нет данных» вместо чисел расхода, а причина
    молчала: файл существовал и был свежим (ревью панели 11.09.2026; замер
    12.09: sudo -u harness-panel cat → Permission denied). Права передаются
    явно: отчёт — читаемый, курсор и сырые данные остаются 0600."""
...
медиана_контекста = процентиль(контексты, 0.50)
п90_контекста = процентиль(контексты, 0.90)
п95_контекста = процентиль(контексты, 0.95)
максимум_контекста = max(контексты) if контексты else 0
...
отчёт = [
    ...
    f"  медиана                   {кратко(медиана_контекста)}",
    f"  90-й процентиль           {кратко(п90_контекста)}",
    f"  95-й процентиль           {кратко(п95_контекста)}",
    f"  максимум                  {кратко(максимум_контекста)}",
    "",
    "Кэш",
...
# подписям («Шаги (обращения к модели)») и очищая их пятью .replace — правка
# подписи оставляла владельца без чисел молча (находка ревью 11.09.2026).
# Отчёт остаётся человеку, машинный файл — потребителям.
write_atomic(machine_path, json.dumps({
    "дата": today,
    "шагов": шагов,
    "с вызовом инструмента": с_вызовами,
    "вызовов на шаг": round(вызовов_всего / с_вызовами, 2) if с_вызовами else None,
    "доля одиночных шагов": round(одиночных / с_вызовами, 4) if с_вызовами else None,
    "контекст на шаг": контекст_на_шаг,
    "медиана контекста": медиана_контекста,
    "90-й процентиль": п90_контекста,
    "95-й процентиль": п95_контекста,
    "максимум контекста": максимум_контекста,
    "кэш прочитано": tot["cache_r"],
    "кэш записано": tot["cache_w"],
    "свежий вход": tot["in"],
    "контекст всего": контекст_всего,
    "доля попаданий кэша": round(tot["cache_r"] / контекст_всего, 4) if контекст_всего else None,
    "ответов всего": tot["out"],
    "ответов на шаг": выход_на_шаг,
    "коммитов": коммитов,
    "прогонов гейтов": гейтов,
    "красных гейтов": красных,
