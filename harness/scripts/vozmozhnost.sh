...
# паспорта) → раскладка пакета (harness/scripts рядом с harness/возможности) →
# раскладка на сервере без конфига (scripts/ рядом с harness/).
caps_dir() {
  ...
  local c
  for c in "${pd:+$pd/harness/возможности}" "$HERE/../vozmozhnosti" "$HERE/../harness/возможности"; do
...
check_requirements() { # $1 = каталог возможности; rc=1 при невыполненном жёстком
  ...
  if (( have_gb >= need_gb )); then
    ...
    say "        адрес: bash harness/demons/disk-cleanup.sh && df -h $home_dir"
    hard_bad=1
  fi
  # память
...
rebuild_skills() { # $1 = каталог проекта; rc=1 — скилы НЕ пересобраны
  local gen="$1/harness/SBORKA-SKILOV.py"
  if [[ ! -f "$gen" ]]; then
  ...
  if python3 "$gen" >/dev/null 2>"$1/.скилы-ошибка"; then
    rm -f "$1/.скилы-ошибка"
    say "  скилы: пересобраны — скилы возможности появились в harness/skills/"
    return 0
  fi
...
cmd_install() { # $1 = id, остальное — флаги установщика
  ...
  if (( reg_rc )); then
    ...
    say "  адрес: python3 <PROJECT_DIR>/harness/SBORKA-SKILOV.py"
...
selftest() {
  ...
  mkdir -p "$T/proj/память" "$T/proj/harness" "$T/log" "$T/caps/проба/skills-src" \
...
EOF
  cat > "$T/proj/harness/SBORKA-SKILOV.py" <<'EOF'
import pathlib
...
EOF
  ...
  [[ -f "$T/proj/harness/скилы-собраны" ]] || { echo "SELFTEST: пересборка скилов не вызвана"; ok_flag=0; }
  ...
  local before after
  before=$(wc -l < "$T/proj/память/MEMORY.md"); : > "$T/proj/harness/скилы-собраны"
