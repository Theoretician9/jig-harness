...
#
# Запуск:
#   bash harness/scripts/unit-paneli.sh            # собрать и поставить
#   bash harness/scripts/unit-paneli.sh --показать # напечатать, ничего не менять
#   bash harness/scripts/unit-paneli.sh --selftest
set -euo pipefail
...
if [ "${1:-}" = "--selftest" ]; then
    ...
    # Требуется ПРИЧИНА словом, а не код 1: проверка шаблона стоит первой, и
    # проба «пустой PROJECT_DIR» зеленела бы и от сломанного пути к шаблону —
    # то есть не от того (ревью кода 12.09.2026, №14).
    PRICHINA_PUSTO=$(sobrat_unit "$T/shablon.in" "" 2>&1 >/dev/null || true)
    proba "БОЛЬНОЙ: пустой PROJECT_DIR — отказ назван словом" "да" \
          "$(printf '%s' "$PRICHINA_PUSTO" | grep -q 'пуст PROJECT_DIR' && echo да || echo нет)"

    PRICHINA_NET=$(sobrat_unit "$T/net-takogo.in" "/srv/подставной" 2>&1 >/dev/null || true)
    proba "БОЛЬНОЙ: нет шаблона — отказ назван словом" "да" \
          "$(printf '%s' "$PRICHINA_NET" | grep -q 'нет шаблона' && echo да || echo нет)"
...
SHABLON="$PROEKT/harness/panel/$UNIT_NAME.in"
