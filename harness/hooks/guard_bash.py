...
#
# Знак «=» в хвосте — третий путь: `local шаг="x"` не ловило НИ ОДНО из двух
# правил (присваивание требует позиции команды, а после `local` её нет;
# объявление требовало пробела после имени). Ровно так кириллическое имя
# прошло в USTANOVIT.sh 13.09.2026 — четвёртый случай за проект.
КИРИЛЛИЦА_В_ОБЪЯВЛЕНИИ = re.compile(
    _CMD_POS + _PRE + r"(?:local|declare|typeset|export|readonly|read)\s+"
    r"(?:-[A-Za-z]+\s+)*(?:[A-Za-z_][A-Za-z0-9_]*(?:=\S*)?\s+)*" + _КИР_ИМЯ
    + r"(?:=(?!=)|\s|;|$)",
    re.MULTILINE,
)
