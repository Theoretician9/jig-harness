...
> `харнес/` из этого пакета (этап 3). Где шаг зависит от раскладки — это
...
(`харнес/scripts/check-secrets.sh`) прогоняется **после раскладки харнеса
...
Раскладываются на этапе 3 (каталог `харнес/`); путь един по всему пакету.
...
не зависят от выбора). Правила канала (контракт для `харнес/scripts/tg_send.sh`,
...
Скопировать на сервер (scp/rsync) два каталога РЯДОМ: `СТАРТОВЫЙ-ПАКЕТ/` и
`UNIFIED/` (например, в `/home/$AGENT_USER/starter/`), затем:

```bash
sudo -u "$AGENT_USER" bash /home/$AGENT_USER/starter/СТАРТОВЫЙ-ПАКЕТ/харнес/разложить-харнес.sh
```

Раскладчик кладёт хуки в `$PROJECT_DIR/scripts/hooks/`, скрипты в `scripts/`,
демоны в `харнес/demons/`, скилы в `харнес/skills/`, доставляет `UNIFIED/` и
не затирает живые данные при повторном запуске.
**Проверка:** `разложить-харнес.sh --selftest` зелёный; вывод раскладки
...
скил `установка-расширений` (`харнес/skills/установка-расширений/`) содержит
...
`харнес/skills-src/установка-расширений.md` + перегенерация скилов.
...
описывается. Такой набор — каталог с паспортом в `харнес/возможности/<id>/`:
...
Единственная точка установки — `scripts/возможность.sh`:

```bash
bash scripts/возможность.sh список                    # что есть, при каком условии
bash scripts/возможность.sh требования <id>           # замер ВЕЩЕЙ до установки
bash scripts/возможность.sh поставить  <id> [--dry-run]
bash scripts/возможность.sh состояние  [<id>]
bash scripts/возможность.sh снять      <id>
```
...
**Проверка (без сервера):** `bash харнес/scripts/возможность.sh --selftest`
зелёный; `bash харнес/возможности/мобильная-разработка/установить.sh --selftest`
зелёный; `python3 харнес/hooks/test_guard_bash.py` зелёный, включая путь
«набор не установлен → его запреты молчат».
**Проверка (на сервере):** `возможность.sh список` показывает набор как «не
стоит»; `харнес/skills/` не содержит `mobile-*` до установки.

## 9. Расписание демонов и heartbeat — после раскладки харнеса (этап 3)

До раскладки `харнес/demons/` этот шаг **пропускается**: cron на
