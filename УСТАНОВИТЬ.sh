#!/usr/bin/env bash
# УСТАНОВИТЬ.sh — установщик харнеса одним скриптом (шаги 0–9 из 01-SPEC-УСТАНОВКА.md).
#
...
#   харнеса (разложить-харнес.sh от agent, с доставкой UNIFIED), harness.conf,
...
# Запуск: sudo bash УСТАНОВИТЬ.sh   (из каталога пакета: рядом харнес/,
...
сверить_демонов_со_спекой() { # $1 = спека; строки crontab — на stdin; rc=1 + перечень при расхождении
  local spec="$1" spec_names cron_names missing extra
  spec_names=$(демоны_из_спеки "$spec")
  if [[ -z "$spec_names" ]]; then
    printf '  в блоке §9 спеки не нашлось ни одной строки $D/<имя>.sh — сверка невозможна\n'
    return 1
  fi
...
if (( SELFTEST )); then
  ...
  проба_пути годен '/home/хозяин/проект-по-русски' 'кириллица в пути законна (в пакете есть харнес/)'
...
[[ -d "$HARNESS_SRC" ]] || { скажи "рядом со скриптом нет каталога харнес/ — запускайте из каталога СТАРТОВЫЙ-ПАКЕТ" >&2; exit 1; }
[[ -f "$HARNESS_SRC/разложить-харнес.sh" ]] || { скажи "нет $HARNESS_SRC/разложить-харнес.sh — пакет неполный" >&2; exit 1; }
...
Environment=PATH=/usr/local/bin:/usr/bin:/bin
WorkingDirectory=$PROJECT_DIR
ExecStart=$PROJECT_DIR/харнес/demons/sentinel.sh
EOF
...
Environment=PATH=/usr/local/bin:/usr/bin:/bin
WorkingDirectory=$PROJECT_DIR
ExecStart=$PROJECT_DIR/харнес/demons/sentinel.sh --boot
TimeoutStartSec=900
...
EOF
  ...
  проверка "$step" "решения сторожа систем доказаны больными случаями" \
    "sudo -u $AGENT_USER bash $PROJECT_DIR/харнес/demons/sentinel.sh --selftest | tail -1 | grep -q зелёный" \
    "sudo -u $AGENT_USER bash $PROJECT_DIR/харнес/demons/sentinel.sh --selftest"
...
шаг_7а() { # доставка и раскладка харнеса (01-SPEC §7-а)
  ...
  local pkg_dst="$starter/СТАРТОВЫЙ-ПАКЕТ"
  ...
  local rollout="$pkg_dst/харнес/разложить-харнес.sh"
  ...
  проверка "$step" "каркас на месте: хуки, скрипты, демоны, скилы" \
    "[ -f '$PROJECT_DIR/scripts/hooks/guard_bash.py' ] && [ -f '$PROJECT_DIR/scripts/tg_send.sh' ] && [ -f '$PROJECT_DIR/харнес/demons/heartbeat-watch.sh' ] && [ -d '$PROJECT_DIR/харнес/skills' ]" \
    "ls '$PROJECT_DIR/scripts' '$PROJECT_DIR/харнес/demons'"
  # harness.conf — раскладчик от agent не имеет прав на /etc; докладываем под root, живой не затираем
  делай "[ -f /etc/harness/harness.conf ] && echo 'harness.conf уже живой — не трогаю' || { cp '$pkg_dst/харнес/config/harness.conf' /etc/harness/harness.conf && chmod 644 /etc/harness/harness.conf; }"
  ...
  делай "cp '$pkg_dst/харнес/config/vhod.sh' /etc/harness/vhod.sh && chmod 644 /etc/harness/vhod.sh"
...
шаг_8() { # минимальный набор расширений (01-SPEC §8): при установке — ничего лишнего
  local step="шаг 8 — минимальный набор расширений"; CURRENT_STEP="$step"
  уже_сделан шаг_8 && { скажи "[$step] пропуск (--resume)"; return 0; }
  заголовок "$step"
  скажи "  по норме UNIFIED/13 при установке НЕ ставится ничего сверх таблицы §8:"
  скажи "  скрипты канала и скилы уже разложены (шаг 7-а);"
  ...
  скажи "  браузер/claude-mem/Context7/example-skills — строго по событиям из таблицы, не сейчас."
  проверка "$step" "скилы и шаблоны разложены (дисциплина пайплайна)" \
    "[ -d '$PROJECT_DIR/харнес/skills' ] && [ -d '$PROJECT_DIR/харнес/шаблоны-задач' ]" \
    "ls '$PROJECT_DIR/харнес'"
  отметить шаг_8
}
...
шаг_9() { # crontab демонов + прогон каждого в env -i (01-SPEC §9)
  ...
  заголовок "$step"
  local D="$PROJECT_DIR/харнес/demons"
  проверка "$step" "демоны разложены (иначе cron — тихий отказ)" \
    "[ -d '$D' ]" \
    "ls '$PROJECT_DIR/харнес' 2>/dev/null || echo 'раскладка не выполнена (шаг 7-а)'"
...
EOF
  ...
  local spec_md="$SCRIPT_DIR/харнес/config/системные-задания.yaml"
  if [[ -f "$spec_md" ]]; then
    ...
    скажи "  OK: набор демонов совпадает с реестром (${#demons[@]} демонов)"
...
осталось_руками() {
  ...
  скажи ""
  скажи "   Прочитай $PROJECT_DIR/харнес/ОПИСЬ.md и файл"
  скажи "   /home/$AGENT_USER/starter/СТАРТОВЫЙ-ПАКЕТ/разработка/03-ЗАДАНИЕ-НОВОМУ-АГЕНТУ.md — и выполняй его"
...
сводка() {
  ...
  скажи ""
  скажи "Дальше: блок «ОСТАЛОСЬ РУКАМИ» выше, затем разработка/ЧЕК-ЛИСТ-ПРИЁМКИ.md (раздел А) и разработка/03-ЗАДАНИЕ-НОВОМУ-АГЕНТУ.md."
  скажи ""
  скажи "Крупные наборы (возможности) НЕ ставятся при установке — только по условию:"
  скажи "   sudo -u $AGENT_USER bash $PROJECT_DIR/scripts/возможность.sh список"
